...
class Repository extends AuthCodeCardConfig {
  ...
  def useCode(userIdentifier: String, cardIdx: Long, codeIdx: Long, code: String): Int = blocking {
  ...
  def getIdentity(userIdentifier: String) = blocking {
    db.withSession { implicit s =>
      authEntriesQuery.filter(line => (line.userIdentifier === userIdentifier)).map(_.identityId).first
    }
  }

  def getNextCardIndex(userIdentifier: String): Long = blocking {
    db.withSession { implicit s =>
      s.withTransaction {
  ...
  def saveAuthEntryAndCodeCard(authEntry: AuthEntry, codeCard: CodeCard) = blocking {
    db.withSession { implicit s =>
      s.withTransaction {
  ...
  def getInactiveCodesForUser(userIdentifier: String): Seq[Code] = blocking {
    db.withSession { implicit s =>
      codesQuery.filter(code => (code.userIdentifier === userIdentifier && code.activatedAt.isEmpty === true)).list
    }
  }

  def activateCode(userIdentifier: String, cardIndex: Long, codeIndex: Long) = blocking {
    db.withSession { implicit s =>
